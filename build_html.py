...
/* CRM dot on card — indexed by pipeline stage */
.crm-dot{{
...
.crm-owner_in  {{background:#3FB950}}
.crm-contract  {{background:#3FB950;box-shadow:0 0 5px #3FB950}}
.crm-won       {{background:#3FB950;box-shadow:0 0 7px #3FB950}}
.crm-lost      {{background:#DA3633}}
/* legacy */
...
/* ── CRM SECTIONS & STAGE SELECTOR ── */
.crm-section{{
  display:flex;flex-direction:column;gap:.55rem;
  padding:.9rem;background:var(--surface2);border:1px solid var(--border);
  border-radius:var(--r-sm);
}}
.crm-section-title{{
  font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;
  color:var(--accent);padding-bottom:.4rem;border-bottom:1px solid var(--border);
}}
.crm-textarea-sm{{min-height:52px}}
...
.stage-btn:hover{{background:var(--surface3);color:var(--text)}}
.stage-btn.active{{
  background:rgba(124,58,237,.15);
  border-color:rgba(124,58,237,.5);
...
/* CRM action buttons */
.crm-log-btn{{
  padding:.5rem .9rem;border-radius:var(--r-sm);font-family:inherit;font-size:.82rem;font-weight:600;
  background:rgba(88,166,255,.12);border:1px solid rgba(88,166,255,.3);
  color:#60A5FA;cursor:pointer;transition:all .15s;
}}
.crm-log-btn:hover{{background:rgba(88,166,255,.22)}}
.crm-obj-btn{{
  padding:.5rem .9rem;border-radius:var(--r-sm);font-family:inherit;font-size:.82rem;font-weight:600;
  background:rgba(210,153,34,.1);border:1px solid rgba(210,153,34,.3);
  color:#FBBF24;cursor:pointer;transition:all .15s;
}}
.crm-obj-btn:hover{{background:rgba(210,153,34,.2)}}

/* Objections */
.obj-row{{
  display:flex;flex-direction:column;gap:.25rem;
  padding:.6rem;background:var(--surface3);border-radius:7px;
...
.obj-quote{{font-size:.85rem;color:var(--text);font-style:italic}}
.obj-handled{{font-size:.78rem;color:var(--text2)}}
.obj-del-btn{{
...
.obj-del-btn:hover{{color:#F85149;background:rgba(218,54,51,.1)}}
...
.hist-row.hist-stage{{color:var(--text);background:rgba(124,58,237,.08);border:1px solid rgba(124,58,237,.2)}}
.hist-icon{{color:var(--accent);flex-shrink:0;font-size:.75rem;margin-top:1px}}
.hist-note{{font-size:.75rem;color:var(--text3)}}
.hist-date{{font-size:.72rem;color:var(--text3);margin-right:auto;white-space:nowrap}}
.empty-hint{{font-size:.8rem;color:var(--text3);padding:.3rem 0}}
...
.crm-modal-header{{
  ...
  font-size:.95rem;font-weight:700;direction:rtl;
  position:sticky;top:0;background:var(--surface);z-index:1;
}}
.crm-modal-header button{{
  background:none;border:1px solid var(--border);
  color:var(--text2);width:28px;height:28px;border-radius:6px;
  cursor:pointer;font-size:.85rem;
}}
.crm-modal-header button:hover{{border-color:var(--danger);color:var(--danger)}}
.crm-modal-body{{
  padding:1rem;display:flex;flex-direction:column;gap:.75rem;direction:rtl;
}}
.crm-stat-chip.crm-chip-red{{border-color:rgba(218,54,51,.3);color:#F85149}}

/* ── PIPELINE PAGE ── */
#pipeline-page{{
  display:none;padding:1.5rem;direction:rtl;
...
.pipeline-title{{font-size:1.3rem;font-weight:800;color:var(--text)}}
.pipeline-summary{{
  display:flex;gap:.65rem;flex-wrap:wrap;
}}
.pip-stat{{
  display:flex;flex-direction:column;align-items:center;
  padding:.5rem .85rem;background:var(--surface2);
  border:1px solid var(--border);border-radius:var(--r-sm);
  min-width:80px;
}}
.pip-num{{font-size:1.4rem;font-weight:800;font-family:'Inter',sans-serif;line-height:1}}
.pip-num.green{{color:#3FB950}}.pip-num.red{{color:#F85149}}
.pip-num.accent{{color:#A78BFA}}.pip-num.gold{{color:#FBBF24}}
.pip-label{{font-size:.65rem;color:var(--text3);margin-top:.2rem;text-align:center}}

.overdue-section{{
  background:rgba(218,54,51,.08);border:1px solid rgba(218,54,51,.25);
  border-radius:var(--r);padding:.85rem 1rem;margin-bottom:1.25rem;
}}
.overdue-title{{font-size:.82rem;font-weight:700;color:#F85149;margin-bottom:.6rem}}
.overdue-list{{display:flex;flex-direction:column;gap:.35rem}}
.overdue-item{{
  display:flex;align-items:center;gap:.75rem;
  font-size:.82rem;cursor:pointer;padding:.35rem .5rem;
  border-radius:6px;transition:background .15s;
}}
.overdue-item:hover{{background:rgba(218,54,51,.12)}}
.overdue-date{{color:#F85149;font-weight:600;white-space:nowrap}}
.overdue-action{{color:var(--text2);font-size:.78rem}}
.overdue-text{{color:#F85149!important}}

.pipeline-board{{
  display:grid;
  grid-template-columns:repeat(auto-fill,minmax(200px,1fr));
  gap:.75rem;margin-bottom:1.5rem;overflow-x:auto;
}}
.pipeline-col{{
  background:var(--surface2);border:1px solid var(--border);
  border-radius:var(--r);overflow:hidden;min-width:180px;
}}
.pipeline-col-header{{
  padding:.65rem .8rem;border-bottom:1px solid var(--border);
  display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;
}}
.pipeline-col-title{{font-size:.78rem;font-weight:700;flex:1}}
.pipeline-col-count{{
  background:var(--surface3);border-radius:20px;
  padding:1px 7px;font-size:.7rem;color:var(--text2);
}}
.pipeline-col-value{{font-size:.68rem;color:#FBBF24;width:100%}}
.pipeline-col-body{{
...
.pipeline-card{{
  ...
  border-radius:7px;padding:.6rem .7rem;cursor:pointer;
  transition:border-color .15s,transform .15s;
  display:flex;flex-direction:column;gap:.2rem;
}}
.pipeline-card:hover{{border-color:rgba(124,58,237,.5);transform:translateX(-2px)}}
.pipeline-card.pipeline-overdue{{border-color:rgba(218,54,51,.4);background:rgba(218,54,51,.05)}}
.pipeline-card-name{{font-size:.82rem;font-weight:700;color:var(--text)}}
.pipeline-card-city{{font-size:.72rem;color:var(--text3)}}
.pipeline-card-value{{font-size:.78rem;color:#FBBF24;font-weight:600}}
.pipeline-card-next{{font-size:.73rem;color:var(--text2)}}
.pipeline-card-date{{font-size:.7rem;color:var(--text3)}}
.pipeline-empty-col{{font-size:.75rem;color:var(--text3);padding:.4rem;text-align:center}}
...
.obj-bars{{display:flex;flex-direction:column;gap:.5rem}}
.obj-bar-row{{display:flex;align-items:center;gap:.5rem;font-size:.78rem}}
.obj-bar-label{{color:var(--text2);min-width:140px;text-align:right}}
.obj-bar-track{{flex:1;height:6px;background:var(--surface3);border-radius:3px;overflow:hidden}}
.obj-bar-fill{{height:100%;background:linear-gradient(90deg,#7C3AED,#58A6FF);border-radius:3px;transition:width .4s}}
.obj-bar-count{{color:var(--text3);font-size:.72rem;width:20px;text-align:left}}
.best-pitch-name{{font-size:1.1rem;font-weight:700;color:var(--text);margin-bottom:.25rem}}
.best-pitch-stat{{font-size:.82rem;color:#3FB950}}
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <button class="nav-item" id="nav-called" onclick="navTo('called')">
      <span class="nav-icon">◈</span>تم الاتصال
    </button>
...
function buildCard(d, idx) {{
  ...
  // Resolve stage: new records have .stage, legacy have .status
  const _legacyMap = {{not_called:'target',no_answer:'contacted',interested:'qualified',follow_up:'discovery',rejected:'lost',closed:'won'}};
  const crmStage = crm?.stage || (crm?.status ? (_legacyMap[crm.status]||'target') : 'target');
  const crmCfg = STAGE_MAP[crmStage] || {{label:'محددة'}};
  ...
  <div class="card-info">
    ...
    <div class="crm-badge-row">
      ...
      <span class="crm-badge-label">${{crmCfg.label || 'محددة'}}</span>
...
function applyFilter() {{
  ...
  let result = ALL.map((d, i) => ({{...d, _origIdx: i}})).filter(d => {{
    ...
    if (needsCRM) {{
      const rec  = crmSnap[d.id];
      const crmSt = rec?.stage || (rec?.status ? ({{not_called:'target',no_answer:'contacted',interested:'qualified',follow_up:'discovery',rejected:'lost',closed:'won'}}[rec.status]||'target') : 'target');
      if (activeChips.has('crm-called')     && crmSt === 'target') return false;
      if (activeChips.has('crm-interested') && !['qualified','discovery','proposal','owner_in','contract','won'].includes(crmSt)) return false;
      if (activeChips.has('crm-followup')   && !['discovery','proposal','owner_in','contract'].includes(crmSt)) return false;
      if (activeChips.has('crm-none')       && crmSt !== 'target') return false;
    }}
    return true;
  }});
...
function navTo(page) {{
  ...
  if (page === 'pipeline') {{
    ...
    if (page === 'home') {{
      ...
      const chipMap = {{
        called:     'crm-called',
        interested: 'crm-interested',
        followup:   'crm-followup',
      }};
      const chipFilter = chipMap[page];
      if (chipFilter) {{
        activeChips.clear();
        document.querySelectorAll('.chip').forEach(c => c.classList.remove('active'));
        const chip = document.querySelector(`.chip[data-filter="${{chipFilter}}"]`);
        if (chip) {{ activeChips.add(chipFilter); chip.classList.add('active'); }}
        applyFilter();
      }}
    }}
  }}
